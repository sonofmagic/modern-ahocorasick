...
- Add pnpm change intents for publishable changes. Release prepares a version PR on ordinary main pushes and publishes when the head commit starts with `chore(release): version packages`. Keep a prepared version PR open when publication is not requested; manual `prepare` and `publish` modes remain available.
- Preserve automatic version PR preparation, publication on version commits and manual fallback modes during repoctl upgrades.
...
- Documentation production is https://aho.icebreaker.top on Workers Static Assets. Keep deploy-docs gated on all CI checks and main, serialize deployments and skip stale commits. Cloudflare credentials stay scoped to the deploy step. Keep npm publication tied to version commits or explicit manual publish runs. Validate clean URLs, 404, deployed SHA and the browser workbench after deployment.
